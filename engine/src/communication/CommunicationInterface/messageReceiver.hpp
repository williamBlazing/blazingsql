...
/* these are utility functions for managing blazing ucp_tags
// which are comprised of 
  uint32_t  message_id;			    The message id which is generated by a global atomic
  ...
  all encoded into a uint64_t
*/

  static ucp_tag_t build_ucp_tag(uint32_t message_id, uint16_t worker_origin_id, uint16_t frame_id);

  static void increment_ucp_tag_frame_id(ucp_tag_t & tag);

  static void set_ucp_tag_frame_id(ucp_tag_t & tag, uint16_t frame_id);
