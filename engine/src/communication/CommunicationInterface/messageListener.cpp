				...
				// io::write_to_socket(connection_fd, &success, sizeof(success));
				{
					...
					while(buffer_position < receiver->num_buffers()) {
						...
						for(size_t chunk = 0; chunk < num_chunks; chunk++) {
							...
							cudaMemcpyAsync(
								buffer_chunk_start, pinned_buffer->data, chunk_size, cudaMemcpyHostToDevice, stream);
						...
						timer_2.reset();
						// TODO: Do we want to do this synchronize and free after all the receiver->num_buffers() or for
						// each one?
					...
					std::cout << "Transfer duration before finish " << duration << " Throughput was "
							  << (((float) total_size) / 1000000.0) / (((float) duration) / 1000.0) << " MB/s"
							  << std::endl;
					...
					std::cout << "META, Recevier, allocate, read, synchronize, total_before_finish, "
							  ...
							  << meta_read_time << ", " << receiver_time << ", " << total_allocate_time << ", "
							  << total_read_time << "," << total_sync_time << ", " << duration << "," << duration_2
							  << ", " << total_size << std::endl;
				}
				cudaStreamSynchronize(stream);
